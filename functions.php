...
function html5blank_scripts()
{
    if (!is_admin()) {
        ...
        wp_register_script('jquery', 'http://ajax.googleapis.com/ajax/libs/jquery/2.0.2/jquery.min.js', array(), '2.0.2'); // Load Google CDN jQuery
        wp_enqueue_script('jquery'); 

        wp_register_script('modernizr', '//cdnjs.cloudflare.com/ajax/libs/modernizr/2.6.2/modernizr.min.js', array(), '2.6.2'); // Modernizr with version Number at the end
        wp_enqueue_script('modernizr'); 

        wp_register_script('selectivizr', '//cdnjs.cloudflare.com/ajax/libs/selectivizr/1.0.2/selectivizr-min.js', array(), '1.0.0'); // HTML5 Blank script with version number
        wp_enqueue_script('selectivizr'); 

        wp_register_script('jquery_preloader',  get_template_directory_uri() . '/assets/js/lib/jquery.preloader-1.2.js', array('jquery'), '1.2'); // HTML5 Blank script with version number
        wp_enqueue_script('jquery_preloader'); 

        wp_register_script('jquery_plugins',    get_template_directory_uri() . '/assets/js/lib/plugins.js', array('jquery'), '1.0.0'); // HTML5 Blank script with version number
...
add_action('init', 'create_post_type_blog'); // Add Post Type
...
add_action('init', 'create_post_type_projects'); // Add Post Type
...
add_action('init', 'create_post_type_labs'); // Add Post Type

function create_post_type_labs()
{
    register_taxonomy_for_object_type('category', 'labs'); // Register Taxonomies for Category
    register_taxonomy_for_object_type('post_tag', 'labs');
    register_post_type('labs', // Register Custom Post Type
        array(
        'labels' => array(
            'name'               => __('Lab Projects', 'labs'), // Rename these to suit
            'singular_name'      => __('Lab Project', 'labs'),
            'add_new'            => __('Add New', 'labs'),
            'add_new_item'       => __('Add New Labs Project', 'labs'),
            'edit'               => __('Edit', 'labs'),
            'edit_item'          => __('Edit Lab Project', 'labs'),
            'new_item'           => __('New Lab Project', 'labs'),
            'view'               => __('View Lab Projects', 'labs'),
            'view_item'          => __('View Lab Project', 'labs'),
            'search_items'       => __('Search Lab Projects', 'labs'),
            'not_found'          => __('No Lab Projects found', 'labs'),
            'not_found_in_trash' => __('No Lab Projects found in Trash', 'labs')
        ),
        'public' => true,
        'hierarchical' => true, // Allows your posts to behave like Hierarchy Pages
        'has_archive' => true,
        ...
        'rewrite' => array( 'slug' => 'labs', 'with_front' => false ),
    ));
    /* flush_rewrite_rules(); */
}
...
require_once('functions_simoncorry.php');
